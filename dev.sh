...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=aplicativo.example.com
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development aplicativo npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop aplicativo
    docker rm aplicativo
    docker run --name aplicativo -d --env-file /home/ubuntu/aplicativo.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/aplicativo:/dkdata \
        aplicativo start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./aplicativo/
...
echo_green "Welcome to aplicativo's dev env"
